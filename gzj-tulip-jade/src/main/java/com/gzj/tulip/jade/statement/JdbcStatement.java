...
import com.gzj.tulip.jade.annotation.AfterInvocation;
...
public class JdbcStatement implements Statement {
    ...
    public JdbcStatement(StatementMetaData statementMetaData, SQLType sqlType,
        ...
        if (sqlType == SQLType.WRITE) {
            ...
            //添加只有没有写SQL语句的才走批量
            //这样更灵活，不然会出现批量的update和insert语句多次调用数据库，性能不好
            SQL sqlAnnotation = method.getAnnotation(SQL.class);
            if (types.length > 0 && List.class.isAssignableFrom(types[0]) && (sqlAnnotation == null // 没有注解@SQL
                    || StringUtils.isBlank(sqlAnnotation.value()))) // 虽注解但没有写SQL
            {
                //TODO 以第一个参数为List类型来判断是否是批量，是否有点草率
