...
public class ConditionalOperationMapper extends OperationMapper {
    ...
    private static final long MODE_ENTITY_CONDITIONS = 1 << 4;

    /**
     * 用于记录mode，因为会出现多个mode的组合
     */
    private List<Class<?>> MODE_LIST = new ArrayList<Class<?>>();
    ...
    protected IParameterMapper createParameterMapper(Class<?> type, Annotation[] annotations, int index) {
        ...
        if (getPrimaryKeyType().isAssignableFrom(type)) {
            MODE_LIST.add(getPrimaryKeyType());
        } else if (getEntityType().isAssignableFrom(type)) {
            MODE_LIST.add(getEntityType());
        } else if (getConditionsType().isAssignableFrom(type)) {
            MODE_LIST.add(getConditionsType());
        }
        ...
        if (annotations.length == 0
            ...
            if (this.mode != MODE_ENTITY_CONDITIONS) {
                if (MODE_LIST.contains(getEntityType()) && MODE_LIST.contains(getConditionsType())) {
                    // 类型为泛型中的实体加条件类型
                    appendMode(MODE_ENTITY_CONDITIONS);
                }
            }
        }
    ...
    public boolean isEntityConditionsMode() {
        return isSpecifiedMode(MODE_ENTITY_CONDITIONS);
    }
