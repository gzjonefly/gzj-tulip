...
import org.mybatis.generator.codegen.mybatis3.javadao.JavaDaoGenerator;
...
public class IntrospectedTableMyBatis3Impl extends IntrospectedTable {
    ...
    protected AbstractJavaClientGenerator calculateClientGenerators(List<String> warnings,
                                                                    ProgressCallback progressCallback) {
        if (!rules.generateJavaDao() && !rules.generateJavaClient()) {
            return null;
        }
    ...
    protected AbstractJavaClientGenerator createJavaClientGenerator() {
        ...
        if ("XMLMAPPER".equalsIgnoreCase(type)) { //$NON-NLS-1$
            ...
            javaGenerator = new JavaMapperGenerator();
        } else if ("DAO".equalsIgnoreCase(type)) { //$NON-NLS-1$
    ...
    protected void calculateJavaModelGenerators(List<String> warnings,
        ...
        if (getRules().generateConditionClass()) {
            AbstractJavaGenerator javaGenerator = new ConditionGenerator();
            initializeAbstractGenerator(javaGenerator, warnings,
                    progressCallback);
            javaModelGenerators.add(javaGenerator);
        }
